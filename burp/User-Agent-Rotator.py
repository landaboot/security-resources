# -*- coding: utf-8 -*-
# ============================================================
#  User-Agent Rotator - Burp Suite Extension (Jython / Python 2.7)
#  Works with: Proxy, Repeater, Intruder, Scanner and any other
#  tool that sends requests through Burp.
#
#  Installation:
...
# ---------------------------------------------------------------------------
# User-Agent list (modern browsers + full pzb/gist list, no bots/CLI tools)
# ---------------------------------------------------------------------------
USER_AGENTS = [
    # ---- Modern browsers (2022-2024) ----
...
class BurpExtender(IBurpExtender, IHttpListener):
    """
    Implements IHttpListener to intercept ALL requests leaving Burp,
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("User-Agent Rotator")
        callbacks.registerHttpListener(self)

        print("[UA Rotator] Extension loaded successfully!")
        print("[UA Rotator] {} User-Agents available.".format(len(USER_AGENTS)))
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        """
        Called for every HTTP message from ANY Burp tool.
        toolFlag values:
          TOOL_PROXY    = 4
          TOOL_SCANNER  = 16
          TOOL_INTRUDER = 32
          ...
          TOOL_SPIDER   = 8
          TOOL_SEQUENCER= 128
          (all are covered since we do not filter by toolFlag)
        """
        ...
        new_headers = self._replace_or_add_header(headers, HEADER_NAME, new_ua)

        new_request = self._helpers.buildHttpMessage(new_headers, body)
        messageInfo.setRequest(new_request)

        print("[UA Rotator] Tool={} | UA={}".format(toolFlag, new_ua))

    # ------------------------------------------------------------------
    # Helper
    # ------------------------------------------------------------------
    def _replace_or_add_header(self, headers, header_name, header_value):
        """
        Replaces the header if it already exists (case-insensitive),
        or appends it if missing.
        headers[0] is always the request-line, so we skip it in the search.
        """
        new_headers  = [headers[0]]  # Keep request-line (e.g. GET / HTTP/1.1)
        header_found = False

        for h in headers[1:]:
            if h.lower().startswith(header_name.lower() + ":"):
                new_headers.append("{}: {}".format(header_name, header_value))
                header_found = True
            else:
